...
for f in sorted(RESULTS_DIR.glob('*_oneshot.json')):
    ...
    cache_file = BASELINE_DIR / f'{name}_easyocr.json'
    ...
    try:
        ...
        (BASELINE_DIR / f'{name}_easyocr_raw.txt').write_text(raw_text)

        numbers = extract_numbers(raw_text)
        m, t = len([c for c in gt_cells if c['value'] in numbers]), len(gt_cells)
...
print(f"\nEASYOCR TOTAL: {total_matched}/{total_cells} = {overall:.1f}%")

with open(BASELINE_DIR / '_easyocr_summary.json', 'w') as fp:
    json.dump({
